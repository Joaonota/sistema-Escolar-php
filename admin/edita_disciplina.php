...
<li class="active"></li>
    ...
    <section class="content">
        <div class="col-xs-12">
  <div class="col-lg-8 col-sm-8 col-xs-12 no-padding edusecArLangCss"><h3 class="box-title"><i class="fa fa-edit"></i> Editar a Disciplina Disciplina</h3></div>
...
$sql_1 = mysqli_query($conexao,"SELECT d.id_disciplina,t.turma,t.id_turma,p.nome,p.id, c.curso,c.id_curso,d.disciplina FROM disciplinas AS d INNER JOIN turmas AS t on turma_id= id_turma INNER JOIN cursos as c on d.cursos_id = c.id_curso INNER JOIN professores AS p on p.id= d.professores_id  WHERE id_disciplina = '$id_disciplina'");
...
$sql_5 = mysqli_query($conexao,"UPDATE  disciplinas set cursos_id = '$cursos_id', disciplina = '$disciplina', professores_id = '$professores_id', turma_id = '$turma_id'");
if($sql_5 == ''){
    echo "<script language='javascript'>window.alert('Ocorreu um erro!');</script>";
}else{
    echo "<script language='javascript'>window.alert('Disciplina Actualizada com sucesso!');window.location='cursoedisciplina.php?pg=cursoedisciplinas';</script>";
  }
...
<select id="batches-batch_course_id" class="form-control" name="cursos_id">
    <option <?php echo $res_1['id_curso']; ?> ><?php echo $res_1['curso']; ?></option>
    <?php
...
<input type="text" id="batches-batch_alias" class="form-control" name="disciplina"maxlength="35" value="<?php echo $res_1['disciplina']; ?>"><p class="help-block help-block-error"></p>
</div>    		
	</div>
			</div>
			<div class="col-xs-12 col-lg-12 no-padding">
...
<p class="help-block help-block-error"></p>
    		...
    		<div class="form-group col-xs-12 col-sm-6 col-lg-4 no-padding edusecArLangCss">
				<div class="col-xs-6">
        			<button type="submit" name="actualizar" class="btn btn-block btn-info">Actualizar</button>				</div>
				<div class="col-xs-6">
